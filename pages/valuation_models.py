...
*Determine the fair value of a stock based on dividend growth expectations.
             Use GGM for stable companies and the H-Model for companies transitioning from high growth to maturity.*
""")
...
with col1:
    d1 = st.number_input("**Expected Next Dividend ($)**",
                            value=2, 
                            help='The dividend expected to be paid in the next year.')
    
with col2:
    k = st.number_input('**Required Rate of Return (%)**', 
                        value=10.0,
                        help="The investor's required return (Cost of Equity).") / 100

with col3:
    g = st.number_input('**Constant Growth Rate (%)**',  
                        value=5.0,  
...
if g >= k:
        ...
        st.error('The growth rate must be strictly less than the required return ($k$).')
        intrinsic_value = 0.0
...
with col3:
    d0 = st.number_input('**Current Dividend ($)**', 
                         min_value=0.01, 
                         value=2.0, 
                         step=0.1, 
                         help='The most recent dividend paid.')
    
    gs = st.number_input('**Short-term (High) Growth (%)**', 
                         min_value=0.0, 
                         value=12.0, 
                         step=0.5, 
                         help='The initial high growth rate.') / 100
    
    gn = st.number_input('**Terminal (Normal) Growth (%)**',
                          value=5.0, 
                          help='The steady-state growth rate after the transition.') / 100
    

with col4:
    k_h = st.number_input('**Required Rate of Return (%)**',
                            value=10.0, 
...
if gn >= k_h:
    container.error('The terminal growth rate must be strictly less than the required return ($k$).')
    intrinsic_value = 0.0
